...
├─ Headers: X-User-ID (mock auth), Idempotency-Key
├─ Body: {amount: 100.00}

AuthMiddleware: Validate request (prototype: uses X-User-ID)
...
├── internal/
│   ├── dbmodel/
│   │   ├── models.go            # Wallet/Transaction/Limit/Audit models
...
│   │   └── interfaces.go        # DAO interfaces (for unit testing)
│   │
│   ├── service/
...
│   │   ├── interfaces.go        # Service interfaces (fraud/limits)
...
│   └── unit/
│       ├── service_test.go
│       └── transaction_service_unit_test.go
│
├── scripts/
...
├── go.mod
├── go.sum
├── openapi.yaml                 # OpenAPI 3.0 spec (Swagger/Redoc compatible)
├── test_api.sh                  # Automated API test script
└── md/                          # Documentation (README/DESIGN/testing guide)
